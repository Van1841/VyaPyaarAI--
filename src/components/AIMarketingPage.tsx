// it takes product data from local storage, generates marketing content using Google Gemini AI, and displays it in a user-friendly format.
...
// Define types for product data and marketing content stored in local storage
...
// Define the structure of the marketing content generated by AI
...
export function AIMarketingPage() {
  // stores product data from local storage.
  const [productData, setProductData] = useState<ProductData | null>(null);
  // stores target audience input from user.
  const [targetAudience, setTargetAudience] = useState("");
  // stores the generated marketing content.
  const [marketingContent, setMarketingContent] = useState<MarketingContent | null>(null);
  // manages loading state while content is being generated.
  const [isGenerating, setIsGenerating] = useState(false);
  const navigate = useNavigate();
  const { toast } = useToast();

  useEffect(() => {
    // Loads product from localStorage on mount.
